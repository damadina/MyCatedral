    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            @if($capitulos->count() > 1)
                <div class="hidden sm:flex-1 sm:space-x-14 sm:justify-center    sm:flex sm:items-center sm:ml-6">

                    @foreach ($capitulos as $capitulo )
                        <div>
                            {{$capitulo->id}}
                            <button  wire:key="item-{{$capitulo->id}}" wire:click.prevent="clickMenu('{{$capitulo->id}}')">{{$capitulo->titulo}}</button>
                        </div>
                    @endforeach
                </div>
            @endif
        ...
        <hr class="hidden sm:flex h-px my-4 bg-gray-200 border-0 dark:bg-gray-700">

        <div class="flex flex-1 items-center justify-center">
            @if($isHome)
                <h1 class="text-center text-catedral tracking-wide text-xs md:text-2xl font font-semibold">{{$capituloSelectedLiteral}}</h1>
            @else
        ...
        <div class=" hidden py-4 bg-white sm:flex justify-center items-center space-x-4 flex-wrap shadow">
            ...
            paso
                    @if($categoria->elementos->count()===0)
                        @continue
                    @endif
            paso2
                        ...
                        @if($categoria->elementos->count()===1)
                            @if($locale=="es")
                                ...
                                <x-dropdown-link  itemprop="item" href="{{route('elementoXX',['slug' => $categoria->elementos[0]->slug])}}"   class="hover:font-semibold hover:text-catedral">
                                    <span itemprop="name" >aa1{{$categoria->elementos->count()}}{{$categoria->title}}</span>
                                </x-dropdown-link>
                            @else
                                ...
                                <x-dropdown-link  itemprop="item" href="{{route('elementoXX',['locale' => $locale,'slug' => $categoria->elementos[0]->slug])}}"   class="hover:font-semibold hover:text-catedral">
                                    <span itemprop="name" >aa2{{$categoria->elementos->count()}}{{$categoria->title}}</span>
                                </x-dropdown-link>
                            @endif
                        @else
paso3 {{$categoria->title}}
                            <x-dropdown  align="left" >

                                <x-slot name="trigger">
                                    <span class="inline-flex rounded-md">
                                        <button type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none focus:bg-gray-50 active:bg-gray-50 transition ease-in-out duration-150">
                                            {{$categoria->elementos->count()}}{{$categoria->title}}
                                            <svg class="ml-2 -mr-0.5 h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                                            </svg>
                                        </button>
