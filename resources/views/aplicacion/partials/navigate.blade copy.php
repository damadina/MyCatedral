                ...
                <x-slot name="content">
                    <ol itemscope itemtype="https://schema.org/BreadcrumbList"class="overflow-auto " >
                        ...
                        @foreach ($exterior as $each )
                            <li itemprop="itemListElement" onclick="miFunc('{{$each->id}}')" itemscope itemtype="https://schema.org/ListItem">
                                @if($locale == "es")
                                    {{-- onclick="miFunc('{{$each->id}}')" --}}
                                    <x-dropdown-link  itemprop="item" href="{{route('elemento',$each)}}"   class="hover:font-semibold hover:text-black">
                                        <span itemprop="name" >9999{{$each->title}}</span>
                                    </x-dropdown-link>
                                @else

                                    <x-dropdown-link  itemprop="item" href="{{route('idioma',['slug' => $each->slug,'locale' => null])}}"   class="hover:font-semibold hover:text-black">
