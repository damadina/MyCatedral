            ...
            @if($capitulos->count() > 1)
                <div class="hidden sm:flex-1 sm:space-x-14 sm:justify-center    sm:flex sm:items-center sm:ml-6">
                    ...
                    @foreach ($capitulos as $capitulo )
                        <div>
                            <button class="text-sm leading-4 font-medium rounded-md text-gray-500 px-2 py-2 {{$capitulo->id == $capituloSelected->id ? ' border-catedral border-2' : '' }}"  wire:key="item-{{$capitulo->id}}" wire:click.prevent="clickMenu('{{$capitulo->id}}')">{{$capitulo->titulo}}</button>
                        </div>
                    @endforeach
                </div>
            @endif
